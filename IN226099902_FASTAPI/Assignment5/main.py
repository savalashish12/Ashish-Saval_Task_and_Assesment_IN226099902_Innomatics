...
def browse_products(
    keyword: str = Query(None, description="Search by product name"),
    sort_by: str = Query('price', regex="^(price|name)$", description="Sort by 'price' or 'name'"),
    order: str = Query('asc', regex="^(asc|desc)$", description="'asc' for low-to-high, 'desc' for high-to-low"),
...
def get_orders_paged(
    ...
    # Step 2: Calculate total pages (Ceiling division)
...
# 1. Place this ABOVE your @app.get("/products/{product_id}") route
@app.get('/products/sort-by-category')
def sort_by_category():
    """
    Sorts products primarily by category (A-Z) 
    and secondarily by price (Low-High).
    """
    # Using sorted() creates a new list and avoids mutating the original 
    # global products list unexpectedly.
...
def search_orders(customer_name: str = Query(...)):
    ...
    # 3. Return results if found
    return {
        'search_keyword': customer_name, 
        'total_found': len(results), 
        'orders': results
    }
# ── Day 5 — Cart ──────────────────────────────────────────────────
...
# Ise hamesha @app.get('/products/{product_id}') ke UPAR rakhein
